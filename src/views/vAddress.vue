<template>
    <div>
      <v-header></v-header>
      <v-bread><span>地址列表</span></v-bread>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="page-title-normal checkout-title">
              <h2><span>运送地址</span></h2>
            </div>
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="item in addressThree" :class="{'boder':item.isDefault===true}">
                    <dl>
                      <dt>{{item.userName}}</dt>
                    ...
                    <div class="addr-opration addr-del">
                      <a href="javascript:;" class="addr-del-btn" @click="showDelModel(item.addressId)">
                        <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                      </a>
                    </div>
                    <div class="addr-opration" v-show="item.isDefault===false">
                      <a href="javascript:;" class="addr-set-default-btn" @click="setDefault(item.addressId)"><i>设为默认地址</i></a>
                    </div>
                    <div class="addr-opration addr-default" v-show="item.isDefault===true">默认地址</div>
                  </li>
                  <li class="addr-new">
              ...
              <div class="shipping-addr-more">
                <a class="addr-more-btn up-down-btn" :class="{'open': more}" href="javascript:;" @click="showMore()">
                  更多
                  <i class="i-up-down">
            ...
            <div class="next-btn-wrap">
              <router-link :to="{name:'orderConfirm',params:{addressId:addressId}}" class="btn btn--m btn--red">下一步</router-link>
            </div>
          </div>
        </div>
      </div>
      <v-footer></v-footer>
      <v-model v-show="addressModel" @close="addressModel=!addressModel">
        <p slot="contentModel" class="modelDscrip">你确定要删除该条邮寄地址吗？</p>
        <template slot="buttonModel">
          <button class="modelBtn" @click="delAddress">确定</button>
          <button class="modelBtn" @click="addressModel=!addressModel">取消</button>
        </template>
      </v-model>
    </div>
</template>
<style scoped>
.boder { border-color:#ee7a23}
.modelDscrip{ font-size: 20px; color: red;margin-top: 50px;}
.modelBtn{ padding:10px 20px;background:red; color:aliceblue;margin-left:50px; margin-right:50px; margin-top:100px}
</style>
<script>
...
import axios from 'axios';
  export default{
      ...
      methods:{
        init(){
          axios.get('/users/addressList').then((res)=>{
            if(res.data.status==='0'){
              ...
              this.addressThree=this.addressList.slice(0,3);
               this.addressList.forEach((item)=>{    //为什么放这可以，拿出来就不行了呢？！！！！！！
        ...
        showMore(){
          this.more=!this.more;
          if(this.addressThree.length<=3){
            this.addressThree=this.addressList
          }else{
            this.addressThree=this.addressList.slice(0,3);
          }
        },
        setDefault(id){
          this.addressId=id;
          axios.post('/users/addressdefault',{
            addressId:id
          }).then((res)=>{
            let data = res.data;
            if(data.status==='1'){
        ...
        delAddress(){
          axios.post('/users/addressDelete',{
